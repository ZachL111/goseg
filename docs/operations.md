...
`goseg` is centered on storage work in Go. The technical target is: Build segmented append-only logs with index rebuild and compaction planning.
...
The score is deterministic. The current threshold is 174. Risk has a penalty of 7, latency has a penalty of 4, and weight contributes a bonus of 2.
